...
pytestmark = [pytest.mark.acceptance, pytest.mark.supplychain, pytest.mark.requires_minio]
...
class TestPathCanonicalization(TransactionTestCase):
    ...
    def test_canon_basic_path(self):
        ...
        assert result == "/artifacts/test/file.json"
    ...
    def test_canon_reject_encoded_slashes(self):
        ...
        assert "Encoded slashes" in str(exc_info.value)
    ...
    def test_canon_enforce_facet_root(self):
        ...
        # Valid facets
        assert canon_path_facet_root("/artifacts/test") == "/artifacts/test"
        assert canon_path_facet_root("/streams/test") == "/streams/test"

        # Invalid facet
        with pytest.raises(ValueError) as exc_info:
            canon_path_facet_root("/invalid/test")
        assert "must start with /artifacts or /streams" in str(exc_info.value)
...
class TestArtifactExistsAcceptance(TransactionTestCase):
    ...
    def test_artifact_exists_invalid_path(self):
        ...
        assert artifact_exists("/artifacts/test%2Ffile") is False

        # Invalid facet
        assert artifact_exists("/invalid/file") is False

        # Path with ..
        assert artifact_exists("/artifacts/../file") is False
...
class TestSeriesHasNewAcceptance(TransactionTestCase):
    ...
    def test_series_has_new_with_model(self):
        ...
        try:
            ...
            series = ArtifactSeries.objects.create(plan=plan, series_key="test-series")
            ...
            assert result is False  # No watermark tracking yet

        except ImportError:
            # Model not available - verify flag-guarded behavior
            result = series_has_new("/streams/test", min_idx=5)
            assert result is False  # Returns False when not wired
...
class TestJsonSchemaAcceptance(TransactionTestCase):
    ...
    def test_json_schema_with_registry(self, mock_read):
        ...
        mock_read.return_value = {"name": "test", "value": 42}
