...
def entry(payload: Dict[str, Any], emit: Callable[[Dict], None] | None = None, ctrl=None) -> Dict[str, Any]:
    ...
    model = params.get("model") or "claude-3-5-sonnet-20241022"

    if emit:
        emit({"kind": "Event", "content": {"phase": "started"}})

    if mode == "mock":
        text = f"Mock Claude response: {messages[-1]['content'][:64] if messages else ''}"
        if emit:
        ...
        if not api_key:
            return {
                ...
                "error": {"code": "ERR_MISSING_SECRET", "message": "ANTHROPIC_API_KEY missing"},
                "meta": {},
            }
        try:
            import litellm
