...
class Command(BaseCommand):
    help = "Scaffold a container-first WS tool (FastAPI WebSocket /run + presigned PUT + process-per-run)."
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(self.style.SUCCESS(f"Scaffolded WS tool at {proc_dir}"))
...
def info(event: str, **fields):
    rec = {"ts":_ts(),"level":"info","event":event,"service":"tool","env":os.getenv("APP_ENV", os.getenv("MODAL_ENVIRONMENT","dev"))}
    rec.update(fields)
