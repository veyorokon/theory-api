"""
Tests for Modal adapter public behavior: canonical output format,
...
class TestModalAdapter(TestCase):
    ...
    def test_duplicate_detection(self, _mock_storage_service):
        ...
        with patch.object(adapter, '_call_generated', return_value=tar_bytes):
            ...
            res = adapter.invoke(
                processor_ref='test/processor@1',
                inputs_json={}, write_prefix='/artifacts/outputs/', execution_id='e',
    ...
    def test_disabled_gate(self):
        ...
        with override_settings(MODAL_ENABLED=False):
            ...
            res = adapter.invoke(
                processor_ref='test/processor@1',
                inputs_json={}, write_prefix='/artifacts/outputs/', execution_id='e',
                registry_snapshot={'processors': {'test/processor@1': {'image': {'oci': 'ghcr.io/x@sha256:1'}, 'runtime': {}, 'secrets': {'required': []}}}},
