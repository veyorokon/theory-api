...
def test_canonicalize_and_selector_trailing_slash():
    assert canonicalize_selector({"kind":"exact","path":"ARTIFACTS//Foo/Bar.txt/"})["path"] == "/artifacts/foo/bar.txt"
...
def test_overlap_matrix_and_plan_scope():
    ...
    assert selectors_overlap({"kind":"prefix","path":"/artifacts/foo/"},{"kind":"exact","path":"/artifacts/foo/bar"})
    assert not selectors_overlap({"kind":"prefix","path":"/artifacts/foo/"},{"kind":"exact","path":"/artifacts/foobar"})
    # prefix vs prefix ancestor/descendant
    assert selectors_overlap({"kind":"prefix","path":"/artifacts/a/b/"},{"kind":"prefix","path":"/artifacts/a/b/c/"})
    assert selectors_overlap({"kind":"prefix","path":"/artifacts/a/b/c/"},{"kind":"prefix","path":"/artifacts/a/b/"})
    # plan scoping test
...
def test_any_overlap_list():
    ...
    held = [
        {"kind": "prefix", "path": "/artifacts/out/frames/"}
    ]
...
def test_overlap_enforcement_when_enabled():
    ...
    with pytest.raises(ValueError, match="overlapping selectors in request"):
        lm.acquire("planA", [
            {"kind": "prefix", "path": "/artifacts/foo/"},
            {"kind": "exact", "path": "/artifacts/foo/bar"}
        ])
