...
class LocalAdapter(RuntimeAdapter):
    ...
    def _invoke_legacy(
        ...
        try:
            ...
            # Execute container
            result = self._execute_container(docker_cmd, timeout_s or registry_spec.get('runtime', {}).get('timeout_s', 300))
    ...
    def _resolve_secrets(self, registry_spec: Dict[str, Any]) -> Dict[str, str]:
        ...
        if missing_required:
            return {
                'status': 'error',
                'error': f'Missing required secrets: {missing_required}'
            }
    ...
    def _execute_container(self, docker_cmd: List[str], timeout_s: int) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            return subprocess.CompletedProcess(
                docker_cmd, 124, '', f'Container execution timed out after {timeout_s}s'
            )
