...
class BaseWsAdapter:
    ...
    def _validate_envelope(self, env: Dict[str, Any], expected_digest: str | None) -> None:
        ...
        if status == "error":
            ...
            raise WsError(f"Processor error: {err.get('code')} {err.get('message')}")
        if status != "success":
            raise WsError(f"Unexpected status: {status}")

        # index_path discipline
        index_path = env.get("index_path") or ""
        if not index_path.endswith("outputs.json"):
            raise WsError("index_path must end with outputs.json")
