...
def test_unit_settings_are_sqlite_and_unittest_module():
    ...
    assert engine == "django.db.backends.sqlite3", f"Unit tests must use SQLite, got engine: {engine}"

    # A couple of cheap hermeticity checks that commonly drift
    email_backend = getattr(settings, "EMAIL_BACKEND", "")
    assert "locmem" in email_backend, f"Unit tests should use locmem email backend, got: {email_backend}"

    caches = getattr(settings, "CACHES", {})
    default_cache = caches.get("default", {}).get("BACKEND", "")
    assert "locmem" in default_cache, f"Unit tests should use locmem cache, got: {default_cache or caches}"
