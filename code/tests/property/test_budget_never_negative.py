...
class TestBudgetProperties(TransactionTestCase):
        ...
        for operation, amount in operations:
            with transaction.atomic():
                ...
                self.assertGreaterEqual(self.plan.reserved_micro, 0, 
                                       f"Reserved micro went negative: {self.plan.reserved_micro}")
                self.assertGreaterEqual(self.plan.spent_micro, 0,
                                       f"Spent micro went negative: {self.plan.spent_micro}")
    ...
    def test_budget_conservation_property(self):
        ...
        for operation, amount in operations:
            with transaction.atomic():
                ...
                self.assertEqual(total_accounted, expected_total,
                    f"Budget not conserved: {total_accounted} != {expected_total}")
    ...
    def test_no_overdraft_property(self):
        ...
        for overdraft_amount in overdraft_attempts:
            ...
            self.assertLessEqual(total_spent, max_spendable,
                f"Overdraft occurred: spent {total_spent} > available {max_spendable}")
    ...
    def test_concurrent_plan_budget_isolation(self):
        ...
        other_plans = []
        for i in range(3):
            other_plans.append(Plan.objects.create(
        ...
        for plan_id, final_total in results:
            ...
            self.assertEqual(final_total, expected_total,
                f"Plan {plan_id} budget not conserved: {final_total} != {expected_total}")
