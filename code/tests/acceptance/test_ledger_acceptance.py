...
class TestLedgerAcceptance(TransactionTestCase):
    ...
    def test_sequence_monotonicity_per_plan(self):
                ...
                with transaction.atomic():
                    ...
                    Event.objects.create(
                        ...
                        this_hash=event_hash(
                            {"event_type": "test.concurrent", "worker_id": worker_id, "seq": next_seq}
                        ),
                        payload={"event_type": "test.concurrent", "worker_id": worker_id, "seq": next_seq},
                    )
        ...
        self.assertEqual(
            len(successful_workers), 1, f"Expected exactly 1 successful worker, got {len(successful_workers)}"
        )
        self.assertEqual(
            len(failed_workers),
            num_workers - 1,
    ...
    def test_hash_chain_continuity(self):
        ...
        # Create first event
        event1_payload = {"event_type": "test.first", "data": "first_event", "plan_id": self.plan.key}
        event1_hash = event_hash(event1_payload)

        event1 = Event.objects.create(
            plan=self.plan, seq=1, prev_hash=None, this_hash=event1_hash, payload=event1_payload
        )

        # Create second event with correct prev_hash
        event2_payload = {"event_type": "test.second", "data": "second_event", "plan_id": self.plan.key}
        event2_hash = event_hash(event2_payload)

        event2 = Event.objects.create(
            plan=self.plan, seq=2, prev_hash=event1.this_hash, this_hash=event2_hash, payload=event2_payload
        )
    ...
    def test_tamper_detection_through_hash_verification(self):
        ...
        event = Event.objects.create(
            plan=self.plan, seq=1, prev_hash=None, this_hash=original_hash, payload=original_payload
        )
    ...
    def test_concurrent_different_plans_no_interference(self):
        ...
        plans = [self.plan]
        for i in range(4):
            plans.append(Plan.objects.create(key=f"concurrent-plan-{i}", reserved_micro=100000, spent_micro=0))
        ...
        def append_events_for_plan(plan: Plan) -> int:
            ...
            for seq in range(1, 4):  # Create 3 events per plan
                Event.objects.create(
                    ...
                    this_hash=event_hash({"event_type": "test.plan_isolation", "plan_id": plan.key, "seq": seq}),
                    payload={"event_type": "test.plan_isolation", "plan_id": plan.key, "seq": seq},
                )
                events_created += 1
