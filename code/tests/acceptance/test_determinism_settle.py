...
class TestDeterminismSettlementAcceptance(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.plan = Plan.objects.create(key="acceptance-plan", reserved_micro=50000, spent_micro=10000)
        self.transition = Transition.objects.create(plan=self.plan, key="transition-1", status="running")
        self.execution = Execution.objects.create(transition=self.transition, attempt_idx=1)
    ...
    def test_multiple_settlements_maintain_hash_chain(self, mock_upload):
        ...
        execution2 = Execution.objects.create(transition=self.transition, attempt_idx=2)
        execution3 = Execution.objects.create(transition=self.transition, attempt_idx=3)
    ...
    def test_concurrent_settlements_maintain_atomicity(self, mock_upload):
        ...
        execution_events = Event.objects.filter(plan=self.plan, payload__execution_id=str(self.execution.id))
    ...
    def test_plan_budget_constraints_respected(self):
        ...
        transition = Transition.objects.create(plan=limited_plan, key="limited-transition", status="running")
