...
def test_import_reachability_checker_runs():
    ...
    result = subprocess.run(
        [sys.executable, "../tests/tools/check_import_reachability.py"],
        cwd=Path(__file__).parent.parent / "code",  # code/ directory
        capture_output=True,
        text=True,
    )
...
def test_vulture_whitelist_format():
    ...
    whitelist_path = Path(__file__).parent.parent / "code" / "vulture_whitelist.py"
